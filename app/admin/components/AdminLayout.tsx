...
const NAV_GROUPS = [
  {
    ...
    items: [
      { href: '/admin/dashboard', icon: 'fa-gauge-high', label: 'Dashboard', desc: 'Start here' },
      {
        ...
        icon: 'fa-heading',
        label: 'Hero text',
        desc: 'Tagline, stats, buttons',
      },
      {
        href: '/admin/home-sections',
        icon: 'fa-pen-to-square',
        label: 'Home sections',
        desc: 'About, offers, CTA text',
      },
      {
        href: '/admin/page-images',
        icon: 'fa-panorama',
        label: 'Photos',
        desc: 'Hero photos for all pages',
      },
      {
        href: '/admin/images',
        icon: 'fa-star',
        label: 'Featured dishes',
        desc: 'Home dishes and photo strip',
      },
      {
        href: '/admin/promo-video',
        icon: 'fa-circle-play',
        label: 'Promo video',
        desc: 'Home video section',
      },
    ],
  },
  {
    ...
    items: [
      {
        ...
        icon: 'fa-utensils',
        label: 'Menu',
        desc: 'A la carte and grill cuts',
      },
      {
        href: '/admin/events',
        icon: 'fa-tags',
        label: 'Promotions',
        desc: 'Offers and events page',
      },
      {
        href: '/admin/catering',
        icon: 'fa-truck-fast',
        label: 'Catering',
        desc: 'Packages and pricing',
      },
      {
        href: '/admin/gallery',
        icon: 'fa-images',
        label: 'Gallery photos',
        desc: 'Photos and story cards',
      },
    ],
  },
];
        ...
        <header className="sticky top-0 z-30 border-b border-[#D4A373]/12 bg-[#0d0905]/95 backdrop-blur">
          <div className="flex min-h-[72px] items-center gap-3 px-4 py-3 sm:px-6">
            ...
            <div className="min-w-0 flex-1">
              ...
              {subtitle && (
                <p className="mt-1 hidden truncate text-xs text-[#C7B8A8] sm:block">{subtitle}</p>
              )}
            </div>
          ...
          {(guide ||
            ...
            <div className="border-b border-[#D4A373]/10 bg-[#100b07] px-4 py-4 sm:px-6">
              <div className="mx-auto flex max-w-7xl flex-col gap-3 lg:flex-row lg:items-stretch">
                  ...
                  <div className="flex-1 rounded-lg border border-[#D4A373]/14 bg-[#130c08] p-4">
                    <div className="flex items-start gap-3">
                      ...
                      <div className="min-w-0">
                        <p className="text-sm font-black uppercase tracking-wide text-[#FFF7ED]">
                          {guide.title}
                        </p>
                        ...
                        <div className="mt-3 flex flex-wrap gap-2">
                          {guide.items.map((item) => (
                            <span
                              key={item}
                              className="rounded bg-[#0d0905] px-2.5 py-1 text-[0.68rem] font-bold text-[#C7B8A8]"
                            >
                              {item}
                            </span>
                          ))}
                ...
                {status &&
                  (status.contentStorage === 'missing' || status.mediaStorage === 'missing') && (
                      ...
                      <div className="flex items-start gap-3">
                        ...
                        <div>
                          <p className="text-sm font-black uppercase tracking-wide text-red-200">
                            Host storage setup needed
                          </p>
                          <p className="mt-1 text-xs leading-5 text-red-100/80">
                            {status.contentStorage === 'missing' &&
                              'Connect Cloudflare R2 so text and data changes save on the hosted site. '}
                            {status.mediaStorage === 'missing' &&
...
export const input =
  'w-full rounded-lg border border-[#D4A373]/25 bg-[#0d0905] px-3 py-2.5 text-sm text-[#FFF7ED] placeholder-[#C7B8A8]/40 transition-colors focus:border-[#fd850b] focus:outline-none focus:ring-1 focus:ring-[#fd850b]/20';
export const btnPrimary =
  'inline-flex items-center justify-center gap-2 rounded-lg bg-[#fd850b] px-4 py-2.5 text-sm font-black uppercase tracking-wide text-black transition-colors hover:bg-[#ff9a2e] disabled:cursor-not-allowed disabled:opacity-50';
export const btnSecondary =
  'inline-flex items-center justify-center gap-2 rounded-lg border border-[#D4A373]/30 px-4 py-2.5 text-sm font-bold text-[#C7B8A8] transition-colors hover:border-[#fd850b] hover:text-[#fd850b]';
export const btnDanger =
  'inline-flex items-center justify-center gap-2 rounded-lg bg-red-900/20 px-3 py-2 text-xs font-bold text-red-400 transition-colors hover:bg-red-600 hover:text-white';
...
export function Field({
  ...
  return (
    <div>
      <label className="mb-1.5 block text-xs font-black uppercase tracking-wider text-[#C7B8A8]">
        {label}
      </label>
      {children}
  ...
  return (
    <div className="overflow-hidden rounded-lg border border-[#D4A373]/15 bg-[#130c08]">
      <div className="flex items-center justify-between border-b border-[#D4A373]/10 px-5 py-4">
        ...
        {count !== undefined && (
          <span className="rounded-full bg-[#fd850b]/15 px-2.5 py-1 text-xs font-black text-[#fd850b]">
            {count}
          </span>
        )}
      </div>
